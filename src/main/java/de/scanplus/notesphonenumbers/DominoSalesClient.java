...
public class DominoSalesClient {
    ...
    public List<AddressLink> loadLinkList(final Integer start, final Integer count) {
        try {
            ...
            List<AddressLink> list = JSONUtils.parseList(is);
            is.close();
            return list;
    ...
    public boolean saveAddressData(AddressData ad, AddressLink al) {
        try {
            ...
            StringEntity se = new StringEntity(JSONUtils.writeToJSON(ad), ContentType.APPLICATION_JSON);
            patchAddressData.setEntity(se);
            LOG.info("PATCH started with: " + JSONUtils.writeToJSON(ad));
    ...
    public AddressData loadAddressData(AddressLink al) {
        try {
            ...
            InputStream is = resp.getEntity().getContent();
            AddressData ad = JSONUtils.parseAddressData(is);
            is.close();
            return ad;
        } catch (URISyntaxException | IOException ex) {
            LOG.error("", ex);
